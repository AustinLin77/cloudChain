 ...
 */
var GesturesUnlock = function (data, navCtrl) {
  this.radius = 20; // 圆的半径
  this.arcColor = '#999999'//'#FFF'; // 圆的颜色
  this.arcDidColor = '#008ec3';//'#FF0000'; // 手指触摸圆的颜色
  this.arcLineWidth = 2; // 圆的粗细
  this.touchColor = '#008ec3'; // 手指触摸的线条颜色
      ...
      canvas.addEventListener('touchstart', function (e) {
        ...
        for (key in pointArr) {
          if (touchX > (pointArr[key].x - thisObj.radius) && touchX < (pointArr[key].x + thisObj.radius) && touchY > (pointArr[key].y - thisObj.radius) && touchY < (pointArr[key].y + thisObj.radius)) {
            ...
            thisObj.dataSource = data;
            thisObj.navCtrlLock = navCtrl;
